...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        if kv_cache is None:
            kv_cache = KVCache()
        kcache,vcache = kv_cache.update(k,v)
        kq = (q @ kcache.transpose(-2,-1))/(x.shape[-1]**0.5)
        q_pos = torch.arange(len(kcache[0])-x.shape[1],len(kcache[0]))
        k_pos = torch.arange(len(kcache[0]))
