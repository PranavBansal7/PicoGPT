...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.wordembedding = nn.Embedding(vocab_size,model_dim)
        self.posembedding = nn.Embedding(context_length,model_dim)
        blocks = [self.TransformerBlock(model_dim,num_heads) for i in range(num_blocks)]
        self.blocks = nn.Sequential(*blocks)
        self.layernormfinal = nn.LayerNorm(model_dim)
        self.vocabproj = nn.Linear(model_dim,vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        pe = torch.arange(context.shape[1])
        x = self.wordembedding(context) + self.posembedding(pe)
        x = self.blocks(x)
